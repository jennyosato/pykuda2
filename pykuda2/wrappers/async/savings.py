...
class AsyncSavings(BaseAsyncAPIWrapper):
    ...
    async def get_plain_savings_account(
        ...
        return await self.api_call(service_type=ServiceType.GET_PLAIN_SAVE, data=data)

    async def get_plain_savings_accounts(self, primary_account_number: str):
    ...
    async def get_fixed_savings_account(
        ...
        return await self.api_call(service_type=ServiceType.GET_FIXED_SAVE, data=data)

    async def get_fixed_savings_accounts(self, primary_account_number):
        data = {"PrimaryAccountNumber": primary_account_number}
        return await self.api_call(
            service_type=ServiceType.GET_ALL_CUSTOMER_FIXED_SAVE, data=data
        )

    async def close_fixed_savings_account(self, amount: int, tracking_reference: str):
        data = {"Amount": amount, "TrackingReference": tracking_reference}
        return await self.api_call(
