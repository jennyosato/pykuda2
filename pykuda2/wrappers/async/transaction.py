...
class AsyncTransaction(BaseAsyncAPIWrapper):
    async def get_banks(self):
        ...
        """
        return await self.api_call(service_type=ServiceType.BANK_LIST)
    ...
    async def confirm_transfer_recipient(
        ...
        return await self.api_call(service_type=ServiceType.NAME_ENQUIRY, data=data)
    ...
    async def get_transaction_history(self, page_size: int, page_number: int):
